...
parser.add_argument("--input_size", type=int, nargs='+', default=[30002], help="Dimensions of 1 input data sample, for nlp it is vocablary size")
...
parser.add_argument("--augment", type=bool, default=True, help="<CNNs /image classification only> Whether to apply basic augmentation")
...
parser.add_argument("--numepochs", type=int, default=2, help="How many epochs to run each model for")
...
parser.add_argument("--bo_prior_states", type=int, default=2, help="# prior states for Bayesian optimization")
parser.add_argument("--bo_steps", type=int, default=2, help="# optimization steps for Bayesian optimization")
...
parser.add_argument("--embedding_dim", type=int, nargs=2, default=[70, 300], metavar=('lower_limit', 'upper_limit'), help="<CNNs / NLP only> Limits for # embedding size")
...
if args.network == 'cnn':
    ...
    run_model_search_mlp(data=data, dataset_code=dataset_code,
                         ...
                         drop_probs=args.drop_probs_mlp, num_best=args.num_best, prior_time=args.prior_time)
    ...
    run_model_search_nlp(data=data, dataset_code=dataset_code,
                         ...
                         num_conv_layers=args.num_conv_layers, channels_first=args.channels_first, channels_upper=args.channels_upper, lr=args.lr, weight_decay=args.weight_decay, batch_size=args.batch_size,
                         bn_fracs=args.bn_fracs, do_fracs=args.do_fracs, input_drop_probs=args.input_drop_probs, drop_probs=args.drop_probs_cnn,
                         num_best=args.num_best, prior_time=args.prior_time, embedding_dim=args.embedding_dim)
